      ...
      <div className="h-full flex flex-col justify-center max-w-2xl mx-auto text-center space-y-10">
        <div className="relative">
          ...
          <div className="relative space-y-6">
            <Award className="w-20 h-20 text-emerald-500 mx-auto drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]" />
        ...
        <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto">
          <GlassCard className="p-5 border-card-border text-center">
            <CircleDot className="w-5 h-5 text-emerald-400 mx-auto mb-2" />
      ...
      <div className="space-y-8 max-w-3xl mx-auto pb-10">
        ...
        <div className="text-center space-y-6 py-8">
          <div className="relative inline-block">
            <div className={`absolute inset-0 blur-[60px] rounded-full scale-150 ${passed ? 'bg-emerald-500/30' : 'bg-red-500/20'}`} />
            <div className="relative">
              {passed ? (
          ...
          <div>
            <h2 className={`text-6xl font-black italic ${passed ? 'text-emerald-400' : 'text-red-400'}`}>
              {percentage}%
            </h2>
            <p className={`text-lg font-bold italic mt-2 ${passed ? 'text-emerald-400/70' : 'text-red-400/70'}`}>
          ...
          {results.map((result, idx) => (
            <GlassCard
              ...
              className={`p-6 border-2 space-y-3 ${
                result.isCorrect ? 'border-emerald-500/20 bg-emerald-500/[0.02]' : 'border-red-500/15 bg-red-500/[0.02]'
              }`}
            >
              <div className="flex items-start gap-3">
                <div className={`p-1.5 rounded-lg shrink-0 mt-0.5 ${
                  result.isCorrect ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/15 text-red-400'
                }`}>
                ...
                <div className="flex-1">
                  ...
                  {/* Show correct answer */}
                  {!result.isCorrect && (
                    <div className="mt-2 text-xs text-emerald-400 italic">
                ...
                <span className={`text-xs font-bold italic shrink-0 ${
                  result.isCorrect ? 'text-emerald-400' : 'text-foreground/20'
                }`}>
            ...
            <GlassCard className="p-8 border-card-border space-y-6">
              ...
              <div className="flex items-center gap-3">
                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest italic border ${
                  current.tipo === 'multiple_choice' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all ${
                          answers[current._id] === optIdx
                            ? 'border-emerald-500 bg-emerald-500/5 text-foreground shadow-[0_0_20px_rgba(16,185,129,0.05)]'
                      ...
                      <button
                        ...
                        className={`flex-1 py-6 rounded-3xl border-2 font-bold italic text-lg transition-all ${
                          answers[current._id] === value
                            ? value
                              ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.08)]'
