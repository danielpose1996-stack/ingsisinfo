          ...
          >
            <div className="text-center max-w-3xl mx-auto space-y-4">
              <h1 className="text-4xl md:text-5xl font-black text-foreground italic tracking-tight">
                Líneas de <span className="text-emerald-400">Aprendizaje</span>
              </h1>
              ...
              {loading ? (
                ...
                modulos.map((mod) => (
                  <GlassCard 
                    key={mod.id} 
                    hover 
                    className="p-8 flex flex-col items-center justify-center text-center group cursor-pointer border-card-border hover:border-emerald-500/30"
                    onClick={() => handleModuleClick(mod)}
                  >
                    <div className="mb-6 p-5 rounded-2xl bg-card group-hover:bg-emerald-500/10 text-foreground/40 group-hover:text-emerald-400 border border-card-border group-hover:border-emerald-500/20 transition-all duration-500 rotate-3 group-hover:rotate-0">
                      {getModuleIcon(mod.nombre.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, '-'))}
                    </div>
                    <h3 className="text-xl font-bold text-foreground mb-3 tracking-tight italic group-hover:text-emerald-500 transition-colors uppercase">
                      {mod.nombre}
                    </h3>
                    <p className="text-foreground/60 text-sm italic leading-relaxed line-clamp-2">
                      {mod.descripcion || 'Profundización en técnicas avanzadas de investigación y desarrollo tecnológico.'}
                    </p>
                    <div className="mt-8 flex items-center gap-2 text-[10px] font-bold text-emerald-500 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
              ...
              <div className="flex bg-card p-1 rounded-2xl overflow-x-auto custom-scrollbar">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-emerald-500 text-black' : 'text-foreground/40 hover:text-foreground hover:bg-card'}`}
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {proyectosLinea.length > 0 ? (
                    proyectosLinea.map(p => (
                      <GlassCard key={p.id} className="p-6 border-card-border hover:border-emerald-500/30 transition-all flex flex-col group">
                        <div className="flex-grow mb-6">
                          ...
                          <h4 className="text-xl font-bold text-foreground italic group-hover:text-emerald-500 transition-colors mb-4 line-clamp-2 uppercase tracking-tight">{p.nombre}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {contenidos[activeTab]?.length > 0 ? (
                    contenidos[activeTab].map((item) => (
                      ...
                      >
                        {activeTab === 'video' ? (
                          <div className="group bg-card rounded-2xl overflow-hidden border border-card-border hover:border-emerald-500/30 transition-all">
                            ...
                            <div className="p-5">
                              <h4 className="text-foreground font-bold italic mb-2 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">{item.titulo}</h4>
                          ...
                          >
                             <div className="w-14 h-14 rounded-2xl bg-card flex items-center justify-center text-foreground/40 mb-6 group-hover:text-emerald-400 transition-colors border border-card-border group-hover:rotate-12 duration-500">
                               <BookOpen className="w-7 h-7" />
                             </div>
                             <h4 className="text-lg font-bold text-foreground italic mb-2 tracking-tight group-hover:text-emerald-400 transition-colors">{item.titulo}</h4>
                             <p className="text-foreground/40 text-[10px] font-bold uppercase tracking-widest italic">Ver Contenido Especializado</p>
                          </GlassCard>
                        ) : (
                          <GlassCard hover className="p-6 flex items-start gap-4 border-card-border hover:border-emerald-500/20 group">
                             <div className="p-3 rounded-xl bg-card text-foreground/40 group-hover:text-emerald-400 border border-card-border group-hover:border-emerald-500/30 transition-all">
                               {activeTab === 'guia' ? <FileText className="w-5 h-5" /> : <LinkIcon className="w-5 h-5" />}
                             </div>
                             <div className="flex-1">
                               <h4 className="text-foreground font-bold italic mb-1 group-hover:text-emerald-500 transition-colors uppercase tracking-tight">{item.titulo}</h4>
                               ...
                               <a 
                                 ...
                                 className="inline-flex items-center gap-2 text-[10px] font-black text-emerald-500 hover:text-emerald-400 uppercase tracking-widest italic"
                               >
          ...
          <div className="w-full lg:w-72 bg-white/[0.02] border-r border-card-border flex flex-col h-full shrink-0">
            <div className="p-6 border-b border-card-border">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.2em] italic">Progreso</span>
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar p-6 space-y-2">
              {ovaSteps.map((step, idx) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl transition-all flex items-center gap-4 group ${activeOvaStep === idx ? 'bg-emerald-500/10 border border-emerald-500/20' : 'hover:bg-card border border-transparent'}`}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-black italic transition-all ${activeOvaStep === idx ? 'bg-emerald-500 text-black' : 'bg-card text-foreground/40 group-hover:text-foreground'}`}>
                    0{idx + 1}
                  </div>
                  <div className="flex-grow min-w-0">
                    <p className={`text-[8px] font-black uppercase tracking-widest italic mb-0.5 ${activeOvaStep === idx ? 'text-emerald-400' : 'text-gray-600'}`}>{step.label}</p>
                    <p className={`text-xs font-bold italic truncate ${activeOvaStep === idx ? 'text-foreground' : 'text-foreground/40'}`}>{step.titulo}</p>
                  </div>
                  {activeOvaStep > idx && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />}
                </button>
              ))}
            </div>
                    ...
                    <div className="space-y-10">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                         <div className="space-y-4">
                           <h4 className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest italic flex items-center gap-3">
                             <div className="w-8 h-px bg-emerald-500/50" /> Objetivo Central
                           </h4>
                    ...
                    <div className="space-y-8 pb-10">
                       <div className="flex items-end justify-between gap-4 border-b border-card-border pb-8">
                         <div className="space-y-2">
                           <h4 className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.3em] font-mono italic">{currentStep.label}</h4>
            ...
            <div className="p-6 border-t border-card-border bg-card/50 flex items-center justify-between">
               ...
               <div className="hidden lg:flex items-center gap-2">
                 {ovaSteps.map((_, i) => (
                   <div 
                     key={i} 
                     className={`h-1 transition-all rounded-full ${i === activeOvaStep ? 'w-8 bg-emerald-500' : 'w-2 bg-white/10'}`} 
                   />
                 ))}
               </div>
